...
def get_running_gaia_sessions() -> Set[int]:
    """Get the set of currently running gaia tmux session partition numbers."""
    running = set()
    try:
        ...
        if result.returncode == 0:
            for line in result.stdout.strip().split("\n"):
                if line.startswith("gaia"):
                    try:
                        partition = int(line.replace("gaia", ""))
...
def start_partition(partition: int) -> bool:
    ...
    session_name = f"gaia{partition}"
    cmd = f"cd /Users/lihengchen/Github/owl && conda activate owl-gaia && python run_gaia_workforce.py --start-idx {config['start']} --end-idx {config['end']} --partition {partition}"
...
def main():
    ...
    while True:
        ...
        # Get current state
        running_sessions = get_running_gaia_sessions()
